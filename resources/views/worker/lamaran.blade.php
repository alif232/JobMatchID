...
<body>
    ...
    <nav class="navbar navbar-expand-lg navbar-light fixed-top py-3 backdrop"
        ...
        <section class="py-0" id="home">
            ...
            <!-- ===============================================-->
            <div class="container">
                <h2 class="mb-4">Data Lamaran Kerja</h2>
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="statusModalLabel">Detail Status Lamaran</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <h5>Posisi: <span id="modalPosisi"></span></h5>
                            <h5>Perusahaan: <span id="modalCompany"></span></h5>
                            <hr>
                            <h5>Riwayat Status:</h5>
                            <ul id="modalStatusList" class="list-group">
                                <!-- Daftar status akan diisi dengan AJAX -->
                            </ul>
                            <div id="modalNote" class="mt-3">
                                <!-- Note akan diisi dengan AJAX -->
                            </div>
                        </div>
...
$(document).ready(function () {
    var table = $('#tbl_list').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "{{ route('worker.lamaran') }}",
            type: 'GET',
        },
        columns: [
            { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false },
            { data: 'posisi', name: 'posisi', orderable: false, searchable: true },
    ...
    $('#tbl_list').on('click', '.detail-status', function () {
        var url = $(this).data('url'); // URL detail status
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#modalPosisi').text(data.lamaran.posisi || 'Tidak tersedia');
                $('#modalCompany').text(data.lamaran.company_name || 'Tidak tersedia');

                // Bersihkan daftar status lama dan note
                $('#modalStatusList').empty();
                $('#modalNote').empty();

                // Tambahkan status baru
                data.status.forEach(function (status) {
                    $('#modalStatusList').append(
                        '<li class="list-group-item">' +
                        status.status + ' - ' +
                        '<span class="badge ' + status.badge_color + '">' + status.created_at + '</span>' +
                        '</li>'
                    );
                });

                // Tampilkan note terbaru
                $('#modalNote').html('<strong>Catatan Terbaru:</strong> ' + data.latest_note);

                // Tampilkan modal
                $('#statusModal').modal('show');
            },
            error: function () {
                alert('Gagal memuat detail status.');
            }
        });
    });
});
